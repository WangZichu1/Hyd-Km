...
from fusion_dataset import loaddata_std
...
for i in range(10):
    ...
    weights_path = f"path/km_weights_fusion{i}.pth"
    scalser_path = f"path/km_scalers_fusion{i}.pkl"
    curve_path = f"path/loss_curves_fusion{i}.png"

    val_prepath = f'path/km_predictions_fusion{i}.npy'
    val_labelpath = f'path/km_val_labels_fusion{i}.npy'

    idx_path = "path/index_fulldata_final_train_10folds.pth"
    full_data_path = "path/fulldata_trainV2.pth"

    train_loader,valid_loader,train_dataset,valid_dataset,wfeat_scaler,pfeat_scaler,sfeat_scaler,label_scaler= loaddata_std(full_data_path,idx_path,fold_id,batch_size=b_s,is_train=True, scaler=None)
    ...
    def train(epoch,train_loader,valid_loader,model,device="cuda:0"):
        ...
        num_batches = 0       
        num_valbatches = 0
        for batch_idx,(wfeats,rpfeats,rsfeats,labels) in enumerate(train_loader):

            wfeatures = wfeats.to(device)
            pfeatures = rpfeats.to(device)
            sfeatures = rsfeats.to(device)
            labels = labels.to(device)
            
            y_hat = model(wfeatures,pfeatures,sfeatures)
        ...
        with torch.no_grad():
            ...
            for batch_idx,(wfeats,rpfeats,rsfeats,labels) in enumerate(valid_loader):

                val_wfeatures = wfeats.to(device)
                val_pfeatures = rpfeats.to(device)
                val_sfeatures = rsfeats.to(device)
                val_labels = labels.to(device)

                val_y_hat = model(val_wfeatures,val_pfeatures,val_sfeatures)
