...
pdb_files = [f for f in os.listdir(data_path) if f.endswith(".pdb") or f.endswith(".ent")]
...
for pdb_file in tqdm(pdb_files, desc="Processing PDB files"):
    original_pdb_id, ext = os.path.splitext(pdb_file)
    file_extension_map[original_pdb_id] = ext
    truncated_pdb_id = original_pdb_id[:25]
...
for original_pdb_id, truncated_pdb_id in tqdm(unique_pdb_id_dict.items(), desc="Copying and renaming PDB files"):
    ...
    src_file = os.path.join(data_path, f"{original_pdb_id}{file_extension_map[original_pdb_id]}")
