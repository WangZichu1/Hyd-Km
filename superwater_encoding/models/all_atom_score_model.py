...
class CrossAttentionLayer(nn.Module):#cross modality attetion
...
class SelfAttentionLayer(nn.Module):#self-attetion
    def __init__(self, embed_dim, num_heads):
...
class TensorProductScoreModel(torch.nn.Module):#main
    def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
        ...
        if self.confidence_mode:#n_lig_nodes, 2*ns] or [n_lig_nodes, ns]
            ...
            # convolution for translational and rotational scores,Step 1 
    ...
    def forward(self, data):
        ...
        ll_distance_cutoff = (tr_sigma * 3 + 20).unsqueeze(1)## [B,1]
        #2.
        ...
        for l in range(self.num_conv_layers):#
            ...
            lr_update = self.conv_layers[9*l+1](rec_node_attr, lr_edge_index, lr_edge_attr_, lr_edge_sh,#shape = [n_lig, out_irreps.dim]
            ...
            ##### 
            water_embeddings = lig_node_attr  

            if l != self.num_conv_layers - 1:  # last layer optimisation，5. 
        ...
        # 6，confidence and affinity prediction， 1：confidence_mode = True
        if self.confidence_mode:
            ...
            return confidence,water_embeddings############
        #7.
        #7.1 
        ...
        #7.4 
        return tr_pred, expand_tr_sigma, data['ligand'].batch,water_embeddings##########
...
class RecycleNet(torch.nn.Module):#
    def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
        ...
        self.score_model = TensorProductScoreModel(t_to_sigma, device, timestep_emb_func, in_lig_edge_features, sigma_embed_dim, sh_lmax,
                                                   ...
                                                   confidence_dropout, confidence_no_batchnorm, num_confidence_outputs, recycle_output_size)#所有其他超参原封不动透传给内部的 score_model
